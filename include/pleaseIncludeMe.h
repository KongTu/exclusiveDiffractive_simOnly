...
double giveMe_t(int option, TLorentzVector e_beam, TLorentzVector e_scattered, TLorentzVector p_beam, TLorentzVector vm_vect){
	...
	if( e_scattered.Eta() < -3.5 ) method_E = 0.01;//photoproduction protection

	double method_A = -99;
	TVector2 sum_pt(vm_vect.Px()+e_scattered.Px(), vm_vect.Py()+e_scattered.Py());
	method_A = sum_pt.Mod2();
	if( e_scattered.Eta() < -3.5 ) method_A = vm_vect.Pt()*vm_vect.Pt();//photoproduction;
	...
	if( e_scattered.Eta() < -3.5 ) method_L = 0.05;//photoproduction protection
